...
def crear_copia_seguridad_zip(origen, destino, nombre_archivo):
    if not os.path.exists(origen):
        messagebox.showerror("Error", f"El directorio de origen '{origen}' no existe.")
        return

    fecha_actual = datetime.now().strftime('%Y%m%d_%H%M%S')
    nombre_zip = f'{nombre_archivo}_{fecha_actual}.zip'
...
def iniciar_copia_seguridad():
    ...
    nombre_archivo = entrada_nombre_archivo.get()
    if not nombre_archivo:
        messagebox.showerror("Error", "Por favor, ingresa un nombre para el archivo de respaldo.")
        return
    crear_copia_seguridad_zip(origen, destino, nombre_archivo)
...
tk.Label(ventana, text="Nombre del archivo de respaldo:").grid(row=2, column=0, padx=10, pady=10)
entrada_nombre_archivo = tk.Entry(ventana, width=50)
entrada_nombre_archivo.grid(row=2, column=1, padx=10, pady=10)
